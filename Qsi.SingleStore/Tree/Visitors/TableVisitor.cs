...
using Qsi.SingleStore.Tree.Common;
...
internal static class TableVisitor
{
    ...
    public static QsiTableNode VisitQueryExpression(in CommonQueryContext context)
    {
        ...
        var node = new SingleStoreDerivedTableNode();
    ...
    public static SingleStoreSelectOptionNode VisitSelectOption(SelectOptionContext context)
    {
        var node = new SingleStoreSelectOptionNode();
        ...
        if (context.children[0] is ITerminalNode terminalNode)
        {
            switch (terminalNode.Symbol.Type)
            {
                case SQL_NO_CACHE_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlNoCache;
                    break;

                case SQL_CACHE_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlCache;
                    break;

                case MAX_STATEMENT_TIME_SYMBOL:
                    node.Option = SingleStoreSelectOption.MaxStatementTime;
        ...
        else
        {
            ...
            {
                case ALL_SYMBOL:
                    node.Option = SingleStoreSelectOption.All;
                    break;

                case DISTINCT_SYMBOL:
                    node.Option = SingleStoreSelectOption.Distinct;
                    break;

                case STRAIGHT_JOIN_SYMBOL:
                    node.Option = SingleStoreSelectOption.StraightJoin;
                    break;

                case HIGH_PRIORITY_SYMBOL:
                    node.Option = SingleStoreSelectOption.HighPriority;
                    break;

                case SQL_SMALL_RESULT_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlSmallResult;
                    break;

                case SQL_BIG_RESULT_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlBigResult;
                    break;

                case SQL_BUFFER_RESULT_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlBufferResult;
                    break;

                case SQL_CALC_FOUND_ROWS_SYMBOL:
                    node.Option = SingleStoreSelectOption.SqlCalcFoundRows;
                    break;
            }
        }
    ...
    public static QsiColumnNode VisitSelectItem(SelectItemContext context)
    {
        ...
        SingleStoreTree.PutContextSpan(derivedColumnNode, context);
    ...
    private static string DeduceColumnName(SelectItemContext context, QsiExpressionNode node)
    {
        ...
        while (treeNode.Children is not null &&
               ...
               SingleStoreTree.IsSimpleParExpr[treeNode])
        {
        ...
        switch (treeNode)
        {
            case IQsiLiteralExpressionNode literal:
                ...
                switch (literal.Value)
                {
                    ...
                    case SingleStoreString { CollateName: null, Kind: SingleStoreStringKind.National or SingleStoreStringKind.Default } singleStoreString:
                    {
                        stringValue = singleStoreString.Value;
                        break;
                    }
                }
    ...
    public static QsiTableNode VisitTableReferenceList(TableReferenceListContext context)
    {
        ...
        for (int i = 1; i < sources.Length; i++)
        {
            ...
            var leftSpan = SingleStoreTree.Span[join.Left.Value];
            var rightSpan = SingleStoreTree.Span[join.Right.Value];

            SingleStoreTree.Span[join] = new Range(leftSpan.Start, rightSpan.End);
    ...
    public static QsiTableNode VisitSingleTable(SingleTableContext context)
    {
        ...
        SingleStoreTree.PutContextSpan(derivedTableNode, context);
    ...
    {
        var node = new SingleStoreTableReferenceNode
        {
    ...
    public static QsiJoinedTableNode VisitJoinedTable(JoinedTableContext context, QsiTableNode left)
    {
        ...
        SingleStoreTree.Span[node] = new Range(
            SingleStoreTree.Span[left].Start,
            SingleStoreTree.Span[node].End
        );
    ...
    {
        var node = new SingleStoreExplicitTableReferenceNode
        {
            Identifier = IdentifierVisitor.VisitTableRef(context.tableRef())
        };
    ...
    public static SingleStoreProcedureAnalyseNode VisitProcedureAnalyseClause(ProcedureAnalyseClauseContext context)
    {
        var node = new SingleStoreProcedureAnalyseNode();
    ...
    public static SingleStoreLockingNode VisitLockingClause(LockingClauseContext context)
    {
        var node = new SingleStoreLockingNode();
        var lockStrengh = context.lockStrengh();

        if (lockStrengh != null)
        {
            node.TableLockType = lockStrengh.HasToken(UPDATE_SYMBOL) ?
                SingleStoreTableLockType.Update : SingleStoreTableLockType.Share;
            ...
            if (lockedRowAction != null)
            {
                node.RowLockType = lockedRowAction.HasToken(SKIP_SYMBOL) ?
                    SingleStoreRowLockType.SkipLocked : SingleStoreRowLockType.NoWait;
            }
        }
        else
        {
            node.TableLockType = SingleStoreTableLockType.ShareMode;
        }
