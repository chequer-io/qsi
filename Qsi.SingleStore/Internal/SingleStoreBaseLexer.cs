...
internal abstract class SingleStoreBaseLexer : Lexer, ISingleStoreRecognizerCommon
{
    #region SqlMode
    ...
    public int serverVersion
    {
        ...
        init
        {
            ...
            _singleStoreVersion = SingleStoreSymbolInfo.numberToVersion(value);
    ...
    protected SingleStoreBaseLexer(ICharStream input, TextWriter output, TextWriter errorOutput) : base(input, output, errorOutput)
    {
    }
    ...
    {
        return Charsets.Contains(text.ToLowerInvariant()) ? SingleStoreLexerInternal.UNDERSCORE_CHARSET : SingleStoreLexerInternal.IDENTIFIER;
    }
    ...
    protected void emitDot()
    {
        ...
        var dotToken = TokenFactory.Create(
            source,
            SingleStoreLexerInternal.DOT_SYMBOL,
            ".",
            Channel,
            TokenStartCharIndex,
            TokenStartCharIndex,
        ...
        var identifierToken = TokenFactory.Create(
            source,
            SingleStoreLexerInternal.IDENTIFIER,
            Text[1..],
            Channel,
    ...
    protected int determineFunction(int proposed)
    {
        if (isSqlModeActive(IgnoreSpace))
        {
            ...
            while (input == ' ' || input == '\t' || input == '\r' || input == '\n')
            {
                ...
                Channel = Hidden;
                Type = SingleStoreLexerInternal.WHITESPACE;
                input = InputStream.LA(1);
            }
        }

        return InputStream.LA(1) == '(' ? proposed : SingleStoreLexerInternal.IDENTIFIER;
    }
    ...
    protected static int determineNumericType(string text)
    {
        ...
        if (length < long_len)
            return SingleStoreLexerInternal.INT_NUMBER;
        ...
        if (negative)
        {
            switch (length)
            {
                case long_len:
                    ...
                    smaller = SingleStoreLexerInternal.INT_NUMBER;
                    bigger = SingleStoreLexerInternal.LONG_NUMBER;
                    break;

                case < signed_longlong_len:
                    return SingleStoreLexerInternal.LONG_NUMBER;

                case > signed_longlong_len:
                    return SingleStoreLexerInternal.DECIMAL_NUMBER;

                default:
                    cmp = signed_longlong_str[1..];
                    smaller = SingleStoreLexerInternal.LONG_NUMBER;
                    bigger = SingleStoreLexerInternal.DECIMAL_NUMBER;
                    break;
            }
        }
        else
        {
            switch (length)
            {
                ...
                case > longlong_len:
                    ...
                    smaller = SingleStoreLexerInternal.ULONGLONG_NUMBER;
                    bigger = SingleStoreLexerInternal.DECIMAL_NUMBER;
                    break;
                ...
                default:
                    cmp = longlong_str;
                    smaller = SingleStoreLexerInternal.LONG_NUMBER;
                    bigger = SingleStoreLexerInternal.ULONGLONG_NUMBER;
                    break;
            }
        }
    ...
    {
        if (!SingleStoreSymbolInfo.isKeyword(name, _singleStoreVersion))
