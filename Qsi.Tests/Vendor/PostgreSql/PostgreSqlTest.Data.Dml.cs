...
public partial class PostgreSqlTest
{
    private static readonly TestCaseData[] InsertTestDatas =
    {
        ...
        new("INSERT INTO actor (actor_id, first_name, last_name) SELECT city_id, city, city FROM city", new[] { "SELECT city_id, city, city FROM city" }, 1),
        ...
        // OVERRIDING
        new("INSERT INTO actor OVERRIDING SYSTEM VALUE VALUES (1, 2, 3, 4)", Array.Empty<string>(), 1),
        new("INSERT INTO actor OVERRIDING USER VALUE VALUES (1, 2, 3, 4)", Array.Empty<string>(), 1),

        // WITH Clause
        new("WITH cte AS (SELECT * FROM actor) INSERT INTO actor SELECT * FROM cte", new[] { "WITH cte AS (SELECT * FROM actor)\nSELECT * FROM cte" }, 1),
    ...
    private static readonly TestCaseData[] UpdateTestDatas =
    {
        ...
        new("UPDATE ONLY actor AS a SET actor_id = 1 WHERE a.actor_id > 10 AND false", new[] { "SELECT * FROM ONLY actor a WHERE a.actor_id > 10 AND false" }, 1),
        new("UPDATE actor AS a SET (actor_id, first_name) = (1, 'Foo') WHERE false", new[] { "SELECT * FROM actor a WHERE false" }, 1),
        new("UPDATE actor AS a SET (actor_id, first_name, last_name, last_update) = (SELECT * FROM actor) WHERE false", new[] { "SELECT * FROM actor a WHERE false" }, 1),
        
        // WITH Clause
        new("WITH cte AS (SELECT * FROM actor) UPDATE actor SET actor_id = cte.actor_id FROM cte WHERE false", new[] { "SELECT * FROM actor WHERE false" }, 1),
        
        // FROM Clause
        new("UPDATE actor SET actor_id = city_id FROM city WHERE false", new[] { "SELECT * FROM actor WHERE false" }, 1),
    ...
    private static readonly TestCaseData[] DeleteTestDatas =
    {
        ...
        new("DELETE FROM ONLY actor AS a", new[] { "SELECT * FROM ONLY actor" }, 1),
        new("DELETE FROM actor AS a WHERE actor_id = 1", new[] { "SELECT * FROM actor a WHERE actor_id = 1" }, 1),

        // WITH Clause
        new("WITH cte AS (SELECT * FROM city) DELETE FROM actor USING cte WHERE actor_id = city_id and false;", new [] { "SELECT * FROM actor", "SELECT * FROM city" }, 1),
    ...
    {
        new("INSERT INTO actor (actor_id, first_name, last_name) SELECT $1, $2, $3 FROM city", new object[] { "city_id", "city", "city" }),
        new("INSERT INTO actor VALUES (default, $1, $2, now())", new object[] { "Mason", "Oh" }),
