...
selectStatement
    ...
    | withClause? querySpecificationNointo unionStatement+
        (
          UNION unionType=(ALL | DISTINCT)?
          (querySpecification | queryExpression)
        )?
        orderByClause? limitClause? lockClause?                     #unionSelect
    | withClause? queryExpressionNointo unionParenthesis+
        (
          UNION unionType=(ALL | DISTINCT)?
          queryExpression
        )?
...
withExpression
    : alias=uid ('(' columns=uidList ')')? AS '(' selectStatement ')'
    ;
...
joinPart
    ...
    | joinOuter JOIN tableSourceItem
        (
          ON expression
          | USING '(' uidList ')'
        )                                                           #outerJoin
    | NATURAL joinOuter? JOIN tableSourceItem                       #naturalJoin
    ;

joinOuter
    : joinType=(LEFT | RIGHT) OUTER?
    ;
...
selectElement
    ...
    | localIdAssign? expression (AS? alias=uid)?                    #selectExpressionElement
    ;
...
aggregateWindowedFunction
    : (AVG | MAX | MIN | SUM)
      '(' aggregator=(ALL | DISTINCT)? functionArg ')'              #totalFunctionCall
    | COUNT '(' (starArg='*' | aggregator=ALL? functionArg) ')'     #countFunctionCall
    | COUNT '(' aggregator=DISTINCT functionArgs ')'                #countDistinctFunctionCall
    | (
        BIT_AND | BIT_OR | BIT_XOR | STD | STDDEV | STDDEV_POP
        | STDDEV_SAMP | VAR_POP | VAR_SAMP | VARIANCE
      ) '(' aggregator=ALL? functionArg ')'                         #logicalFunctionCall
    | GROUP_CONCAT '('
...
functionArg
    ...
    | fullColumnName                                                #fullColumnNameArg 
...
predicate
    ...
    | localIdAssign? expressionAtom                                 #expressionAtomPredicate
    ;
...
keywordsCanBeId
    ...
    | ALWAYS | CAST | RECURSIVE | DATE | TIME | TIMESTAMP | DATETIME | YEAR | NVARCHAR | NATIONAL | TEXT | ENUM | AVG 
    | BIT_AND | BIT_OR | BIT_XOR | COUNT | GROUP_CONCAT | MAX | MIN | STD | STDDEV | STDDEV_POP | STDDEV_SAMP | SUM 
    | VAR_POP | VAR_SAMP | VARIANCE | CURDATE | CURTIME | DATE_ADD | DATE_SUB | EXTRACT | NOW | POSITION | SUBSTR 
    | SUBSTRING | SYSDATE | TRIM | INVISIBLE | VISIBLE | EUR | USA | JIS | ISO | QUARTER | MONTH | DAY | HOUR | MINUTE 
    | WEEK | SECOND | MICROSECOND | TABLES | ROUTINE | EXECUTE | FILE | PROCESS | RELOAD | SHUTDOWN | SUPER | PRIVILEGES 
    | ARMSCII8 | ASCII | BIG5 | CP1250 | CP1251 | CP1256 | CP1257 | CP850 | CP852 | CP866 | CP932 | DEC8 | EUCJPMS 
    | EUCKR | GB2312 | GBK | GEOSTD8 | GREEK | HEBREW | HP8 | KEYBCS2 | KOI8R | KOI8U | LATIN1 | LATIN2 | LATIN5 
    | LATIN7 | MACCE | MACROMAN | SJIS | SWE7 | TIS620 | UCS2 | UJIS | UTF16 | UTF16LE | UTF32 | UTF8 | UTF8MB3 
    | UTF8MB4 | ARCHIVE | BLACKHOLE | CSV | FEDERATED | INNODB | MRG_MYISAM | MYISAM | NDB | NDBCLUSTER 
    | PERFORMANCE_SCHEMA | TOKUDB | REPEATABLE | COMMITTED | UNCOMMITTED | SERIALIZABLE | GEOMETRYCOLLECTION 
    | GEOMCOLLECTION | GEOMETRY | LINESTRING | MULTILINESTRING | MULTIPOINT | MULTIPOLYGON | POINT | POLYGON | ABS 
    | ACOS | ADDDATE | ADDTIME | AES_DECRYPT | AES_ENCRYPT | AREA | ASBINARY | ASIN | ASTEXT | ASWKB | ASWKT 
    ...
    | CHAR_LENGTH | COERCIBILITY | COLLATION | COMPRESS | CONCAT | CONCAT_WS | CONNECTION_ID | CONV | CONVERT_TZ | COS 
    | COT | CRC32 | CREATE_ASYMMETRIC_PRIV_KEY | CREATE_ASYMMETRIC_PUB_KEY | CREATE_DH_PARAMETERS | CREATE_DIGEST 
    | CROSSES | DATEDIFF | DATE_FORMAT | DAYNAME | DAYOFMONTH | DAYOFWEEK | DAYOFYEAR | DECODE | DEGREES | DES_DECRYPT 
    | DES_ENCRYPT | DIMENSION | DISJOINT | ELT | ENCODE | ENCRYPT | ENDPOINT | ENVELOPE | EQUALS | EXP | EXPORT_SET 
    | EXTERIORRING | EXTRACTVALUE | FIELD | FIND_IN_SET | FLOOR | FORMAT | FOUND_ROWS | FROM_BASE64 | FROM_DAYS 
    | FROM_UNIXTIME | GEOMCOLLFROMTEXT | GEOMCOLLFROMWKB | GEOMETRYCOLLECTIONFROMTEXT | GEOMETRYCOLLECTIONFROMWKB 
    | GEOMETRYFROMTEXT | GEOMETRYFROMWKB | GEOMETRYN | GEOMETRYTYPE | GEOMFROMTEXT | GEOMFROMWKB | GET_FORMAT 
    | GET_LOCK | GLENGTH | GREATEST | GTID_SUBSET | GTID_SUBTRACT | HEX | IFNULL | INET6_ATON | INET6_NTOA | INET_ATON 
    | INET_NTOA | INSTR | INTERIORRINGN | INTERSECTS | ISCLOSED | ISEMPTY | ISNULL | ISSIMPLE | IS_FREE_LOCK | IS_IPV4 
    | IS_IPV4_COMPAT | IS_IPV4_MAPPED | IS_IPV6 | IS_USED_LOCK | LAST_INSERT_ID | LCASE | LEAST | LENGTH | LINEFROMTEXT 
    | LINEFROMWKB | LINESTRINGFROMTEXT | LINESTRINGFROMWKB | LN | LOAD_FILE | LOCATE | LOG | LOG10 | LOG2 | LOWER | LPAD 
    | LTRIM | MAKEDATE | MAKETIME | MAKE_SET | MASTER_POS_WAIT | MBRCONTAINS | MBRDISJOINT | MBREQUAL | MBRINTERSECTS 
    | MBROVERLAPS | MBRTOUCHES | MBRWITHIN | MD5 | MLINEFROMTEXT | MLINEFROMWKB | MONTHNAME | MPOINTFROMTEXT 
    | MPOINTFROMWKB | MPOLYFROMTEXT | MPOLYFROMWKB | MULTILINESTRINGFROMTEXT | MULTILINESTRINGFROMWKB 
    | MULTIPOINTFROMTEXT | MULTIPOINTFROMWKB | MULTIPOLYGONFROMTEXT | MULTIPOLYGONFROMWKB | NAME_CONST | NULLIF 
    | NUMGEOMETRIES | NUMINTERIORRINGS | NUMPOINTS | OCT | OCTET_LENGTH | ORD | OVERLAPS | PERIOD_ADD | PERIOD_DIFF | PI 
    | POINTFROMTEXT | POINTFROMWKB | POINTN | POLYFROMTEXT | POLYFROMWKB | POLYGONFROMTEXT | POLYGONFROMWKB | POW 
    | POWER | QUOTE | RADIANS | RAND | RANDOM_BYTES | RELEASE_LOCK | REVERSE | ROUND | ROW_COUNT | RPAD | RTRIM 
    | SEC_TO_TIME | SESSION_USER | SHA | SHA1 | SHA2 | SIGN | SIN | SLEEP | SOUNDEX | SQL_THREAD_WAIT_AFTER_GTIDS | SQRT 
    | SRID | STARTPOINT | STRCMP | STR_TO_DATE | ST_AREA | ST_ASBINARY | ST_ASTEXT | ST_ASWKB | ST_ASWKT | ST_BUFFER 
    | ST_CENTROID | ST_CONTAINS | ST_CROSSES | ST_DIFFERENCE | ST_DIMENSION | ST_DISJOINT | ST_DISTANCE | ST_ENDPOINT 
    | ST_ENVELOPE | ST_EQUALS | ST_EXTERIORRING | ST_GEOMCOLLFROMTEXT | ST_GEOMCOLLFROMTXT | ST_GEOMCOLLFROMWKB 
    | ST_GEOMETRYCOLLECTIONFROMTEXT | ST_GEOMETRYCOLLECTIONFROMWKB | ST_GEOMETRYFROMTEXT | ST_GEOMETRYFROMWKB 
    | ST_GEOMETRYN | ST_GEOMETRYTYPE | ST_GEOMFROMTEXT | ST_GEOMFROMWKB | ST_INTERIORRINGN | ST_INTERSECTION 
    | ST_INTERSECTS | ST_ISCLOSED | ST_ISEMPTY | ST_ISSIMPLE | ST_LINEFROMTEXT | ST_LINEFROMWKB | ST_LINESTRINGFROMTEXT 
    | ST_LINESTRINGFROMWKB | ST_NUMGEOMETRIES | ST_NUMINTERIORRING | ST_NUMINTERIORRINGS | ST_NUMPOINTS | ST_OVERLAPS 
    | ST_POINTFROMTEXT | ST_POINTFROMWKB | ST_POINTN | ST_POLYFROMTEXT | ST_POLYFROMWKB | ST_POLYGONFROMTEXT 
    | ST_POLYGONFROMWKB | ST_SRID | ST_STARTPOINT | ST_SYMDIFFERENCE | ST_TOUCHES | ST_UNION | ST_WITHIN | ST_X | ST_Y 
    | SUBDATE | SUBSTRING_INDEX | SUBTIME | SYSTEM_USER | TAN | TIMEDIFF | TIMESTAMPADD | TIMESTAMPDIFF | TIME_FORMAT 
    | TIME_TO_SEC | TOUCHES | TO_BASE64 | TO_DAYS | TO_SECONDS | UCASE | UNCOMPRESS | UNCOMPRESSED_LENGTH | UNHEX 
    | UNIX_TIMESTAMP | UPDATEXML | UPPER | UUID | UUID_SHORT | VALIDATE_PASSWORD_STRENGTH | VERSION 
    | WAIT_UNTIL_SQL_THREAD_AFTER_GTIDS | WEEKDAY | WEEKOFYEAR | WEIGHT_STRING | WITHIN | YEARWEEK | X_FUNCTION 
    | Y_FUNCTION 
    ;
